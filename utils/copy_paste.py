...
def main(args):
    # input path
    segclass = os.path.join(args.input_dir, "masks")
    ...
    for mask_path in tbar:
        ...
        mask_src = np.asarray(
            Image.open(os.path.join(segclass, mask_path)), dtype=np.uint8
        )
        ...
        mask_main = np.asarray(
            Image.open(os.path.join(segclass, mask_main_path)), dtype=np.uint8
        )
        img_main = cv2.imread(
            os.path.join(JPEGs, mask_main_path.replace(".png", ".jpg"))
        )
        ...
        save_colored_mask(mask, os.path.join(args.output_dir, "masks", mask_filename))
...
def get_args():
    ...
    parser.add_argument(
        "--input_dir",
        default="/opt/ml/input/data/trash_train",
        type=str,
        help="input annotated directory",
    )
    parser.add_argument(
        "--output_dir",
        default="/opt/ml/input/data/trash_train_copy_paste",
        type=str,
        help="output dataset directory",
    )
    parser.add_argument(
        "--lsj", default=True, type=bool, help="if use Large Scale Jittering"
    )
