...
# https://github.com/boostcampaitech3/level2-semantic-segmentation-level2-cv-05/blob/main/utils/soft_voting.py
...
def main():
    ...
    for index, image_info in enumerate(tqdm(images, total=len(images))):
        ...
        pred = predictor(img)
        output = (
            pred["sem_seg"].unsqueeze(dim=0).detach().cpu().numpy().astype(np.float16)
        )
